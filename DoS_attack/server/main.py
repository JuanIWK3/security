...
class FibonacciRequestHandler(BaseHTTPRequestHandler):
    ...
    @rate_limit(limit=10, interval=60)
    def do_GET(self):
        try:
            if self.path == "/":
                self._handle_home()
    ...
    def _handle_fib(self, number_str: str):
        ...
        if number > 1000:
            return self._send_response(400, "Number too large")
...
class FibonacciServer:
    ...
    def start(self):
        print(
            colored(f"Server started at http://{self.host}:{self.port}", "yellow"))
        self.server = HTTPServer(
            (self.host, self.port), FibonacciRequestHandler)
